...
export default class PostRepository {
    ...
    findLike = async(ids) => {
        const likefind = await this.Likes.findOne({
            where: { postId: ids.postId, userId: ids.userId }
        });
        return likefind;
    }
    ...
    addLike = async(ids) => {
        await this.Likes.create({ postId: ids.postId, userId: ids.userId });
        await this.Likes.increment({"likes": 1}, {where: { postId: ids.postId }});
        return "add";
    }
    ...
    findLikes = async(userId) => {
        const findAllLike = await this.Likes.findAll({
            where: { userId },
    ...
    countLikes = async(postId) => {
        const countLikes = await this.Likes.count({
            where: { postId },
        });
        return countLikes;
    }
